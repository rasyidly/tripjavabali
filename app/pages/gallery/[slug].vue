...
const { data: doc, pending } = await useAsyncData('galleries', async () => {
    const collection = (`content_${locale.value}`) as keyof Collections
    const data = await queryCollection(collection).path(slug.value).first()
...
useSeoMeta({
    ...
    ogImage: () => doc.value?.meta.image || '/og-image.jpg'
})
</script>
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <div class="sticky top-16 z-40 bg-white/80 dark:bg-gray-900/80 backdrop-blur-lg border-b border-gray-200 dark:border-gray-800">
            <UContainer class="py-4">
                <UButton to="/#gallery" variant="ghost" icon="i-heroicons-arrow-left" size="sm">
                    Back to Gallery
                </UButton>
            </UContainer>
        </div>
        ...
        <UContainer class="py-8 md:py-12">
            ...
            <div v-else-if="doc" class="max-w-4xl mx-auto">
                ...
                <div class="relative mb-8 rounded-2xl overflow-hidden shadow-2xl">
                    <img v-if="doc.meta.image" :src="doc.meta.image" :alt="doc.title" class="w-full h-64 md:h-96 lg:h-[500px] object-cover" />
                        ...
                        <div class="sticky top-32 space-y-6">
                            ...
                            <UCard>
                                <div class="text-center">
                                    ...
                                    <UButton :to="whatsappUrl" target="_blank" color="success" size="lg" icon="i-simple-icons-whatsapp" block>
                                        Book Similar Tour
                                    </UButton>
                                </div>
                            </UCard>
                            ...
                            <UCard>
                                ...
                                <div class="space-y-3">
                                    <NuxtLink v-for="dest in relatedDestinations" :key="dest" to="/#destinations" class="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
